...
func (service *Service) Save(transaction *sql.Tx) (err error) {
	if service.Id == 0 {
		...
		serviceModel := &Service{Id: service.Id}
		serviceModel, err = serviceModel.GetById(transaction)
		if err == sql.ErrNoRows {
			...
			_, err = transaction.Exec("update service set name = $1, price = $2 where id = $3", serviceModel.Name, service.Price, service.Id)
			if err != nil {
				return err
			}
		}
